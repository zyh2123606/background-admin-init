...
import { checkKeytToPath, urlToList, getMenuKeys, getDefaultOpenKeys } from './menuUtils'
...
const BaseMenu: FunctionComponent = () => {
	...
	const [openKeys, setOpenKeys] = useState(getDefaultOpenKeys(menuKeys, location.pathname))
	useEffect(() => {
		setOpenKeys(getDefaultOpenKeys(menuKeys, location.pathname))
	}, [location.pathname])
	//渲染菜单
	const renderMenu = (menuData: RouteProps[] | undefined) => {
		if (!Array.isArray(menuData) || !menuData.length) return []
		const menuList = menuData.map((item: RouteProps) => {
			...
			const menuItem = <MenuItem key={item.path}>{getItemPath(item)}</MenuItem>
	...
	const handleOpenChange = (openKeys: string[]): void => {
		const isMoreThanOne: boolean = openKeys.filter(openKey => isMainMenu(openKey)).length > 1
		openKeys = (isMoreThanOne ? [openKeys.pop() || ''] : [...openKeys]).filter(item => item !== '')
	...
	const getSelectedKeys = (): string[] => {
		return urlToList(location!.pathname).map(path => {
